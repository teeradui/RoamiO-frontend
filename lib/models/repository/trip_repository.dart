...
class TripRepository {
  ...
  Future<Trip> insert(Trip trip, File? image) async {
    ...
    print('Trip name from model: ${trip.tripName}');
    ...
    print(
      'Multipart files: ${request.files.map((file) => file.filename).toList()}',
    );
  ...
  Future<List<Trip>> findByStatus(
    ...
    if (response.statusCode != 200) {
      throw Exception(
        'Failed to load $status trips (${response.statusCode}): ${response.body}',
      );
    }
  ...
  Future<Trip> findById(String tripId, {bool forceRefresh = false}) async {
    ...
    if (response.statusCode != 200) {
      throw Exception(
        'Failed to load trip (${response.statusCode}): ${response.body}',
      );
    }
  ...
  Future<Trip> update(
    ...
    debugPrint('========== PATCH TRIP REQUEST ==========');
    debugPrint('URL: $uri');
    debugPrint('tripId: $tripId');
    debugPrint('raw fields: $fields');
    ...
    fields.forEach((key, value) {
      if (value != null) {
        ...
        debugPrint('PATCH FIELD: $key = ${value.toString()}');
      }
    });
    ...
    debugPrint(
      'multipart files: '
      '${request.files.map((file) => file.filename).toList()}',
    );
